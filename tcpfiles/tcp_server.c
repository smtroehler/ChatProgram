...
main(int argc, char *argv[])
{
 ...
 printf("client socket fd: %d", client_socket);
    ...
    printf("Data: %s\n", buf);
        ...
        int client_socket2= tcp_listen(server_socket, 5);
 printf("client socket 2 fd: %d", client_socket2);
    //now get the data on the client_socket
    if ((message_len = recv(client_socket2, buf, buffer_size, 0)) < 0)
      {
  perror("recv call");
  exit(-1);
      }
